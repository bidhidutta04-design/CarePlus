...
router.get("/", (req, res) => {
  ...
  const list = db.beds.filter((b) => (!ward || b.ward === ward) && (!status || b.status === status));
  const occupied = list.filter((b) => b.status === "Occupied").length;
  res.json({ data: list, meta: { total: list.length, occupied, occupancyPct: list.length ? Math.round((occupied / list.length) * 100) : 0 } });
});

// PATCH /api/beds/:id — admit / transfer / release
router.patch("/:id", requireRole("Admin", "Nurse", "Doctor"), validate(updateSchema), (req, res, next) => {
