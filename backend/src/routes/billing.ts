...
router.get("/", (req, res) => {
  ...
  const list = db.invoices.filter((i) => (!status || i.status === status) && (!patientId || i.patientId === patientId));
  const billed = list.reduce((s, i) => s + i.totalAmount, 0);
  const collected = list.reduce((s, i) => s + i.paidAmount, 0);
  res.json({ data: list, meta: { total: list.length, billed, collected, pending: billed - collected } });
});

// POST /api/billing/invoices
router.post("/invoices", requireRole("Admin", "Cashier"), validate(createSchema), (req, res, next) => {
...
router.post("/:id/collect", requireRole("Admin", "Cashier"), validate(collectSchema), (req, res, next) => {
