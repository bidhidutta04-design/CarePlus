...
import { seedInventory } from "@/lib/seed-data";
import { Boxes, TriangleAlert, IndianRupee } from "lucide-react";

export default function InventoryPage() {
  const low = seedInventory.filter((i) => i.stock <= i.minThreshold);
  const value = seedInventory.reduce((s, i) => s + i.stock * i.unitCost, 0);
