...
export default function ReportsPage() {
  ...
  const occupancy = beds.length > 0 ? Math.round((beds.filter((b) => b.status === "Occupied").length / beds.length) * 100) : 0;
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <KpiCard icon={TrendingUp} label="Total billed" value={formatINR(billed)} rawValue={billed} formatValue={(n) => formatINR(Math.round(n))} sub={`${invoices.length} invoices`} tone="green" />
        <KpiCard icon={BedDouble} label="Bed occupancy" value={`${occupancy}%`} rawValue={occupancy} formatValue={(n) => `${Math.round(n)}%`} sub={`${beds.filter((b) => b.status === "Occupied").length}/${beds.length} beds occupied`} tone="blue" />
